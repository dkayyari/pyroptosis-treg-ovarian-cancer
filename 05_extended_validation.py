...
Description:
    Validates all key findings in two independent GEO cohorts:
      GSE26193 — 107 OC patients, Institut Curie, France
      GSE9891  — 285 OC patients, Australian Ovarian Cancer Study

    Validates:
      1. Pyroptosis-Treg correlation (primary finding)
      2. CCL22/CCR8 mechanism correlations
      3. Immune checkpoint expression differences

    Method: Affymetrix probe IDs are mapped to gene symbols
    using the mygene.info REST API. Results are cached to avoid
    re-running the API call (takes ~20 minutes first time).

Inputs:
    dataraw/GSE26193_series_matrix.txt  — download from GEO
    dataraw/GSE9891_series_matrix.txt   — download from GEO
    data_clean/ssgsea_scores.csv        — from Script 02
    data_clean/TCGA_expr_clean.csv      — from Script 01

Outputs:
    data_clean/Validation_GSE9891_scatter.png
    data_clean/Figure5_checkpoint_validation.png
    data_clean/GSE26193_gene_expr.csv   — cached gene matrix
    data_clean/GSE9891_gene_expr.csv    — cached gene matrix

Data download:
    GSE26193: https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE26193
    GSE9891:  Run in terminal:
...
Requirements:
    pip install pandas numpy matplotlib seaborn scipy requests lifelines
=============================================================
"""
...
EXECUTOR_GENES  = ["GSDMD","CASP1","IL1B","IL18","NLRP3","AIM2"]
RECRUITER_GENES = ["CCL22","CCL17","CCR8","IL33","TGFB1","FOXP3"]
CHECKPOINT_GENES = {
...
def parse_geo(filepath):
    """Parse a GEO series matrix file into expression and metadata."""
    ...
    data_lines = [l for l in lines[data_start:]
                  if not l.startswith("!series_matrix_table_end")]
...
def map_probes_to_genes(expr_df, cohort_name):
    """
    Map Affymetrix probe IDs to HGNC gene symbols using mygene.info API.
    Results are cached to avoid re-querying the API.
    """
    ...
    print(f"  Mapping probes for {cohort_name} via mygene.info API...")
    print(f"  (This takes ~20 minutes — results will be cached)")
    ...
    for i in range(0, min(len(probe_ids), 60000), 1000):
        ...
        try:
            resp = requests.post(
                ...
                data={"q":  ",".join(str(b) for b in batch),
                      "scopes": "reporter",
                      "fields": "symbol",
                      "species":"human"},
                timeout=30
            ...
            print(f"  API error at batch {i//1000+1}: {e}")

        if i % 10000 == 0:
            print(f"    Batch {i//1000+1}: {len(gene_map)} probes mapped")
    ...
    print(f"  Gene matrix: {expr_gene.shape}  (cached to {cohort_name}_gene_expr.csv)")
    return expr_gene
...
def stratify_patients(pyro, treg):
    """Stratify patients into 4 groups by median split."""
    df = pd.DataFrame({"Pyroptosis": pyro, "Treg": treg}).dropna()
    pm = df["Pyroptosis"].median()
    tm = df["Treg"].median()
...
# =============================================================
# LOAD AND VALIDATE BOTH GEO COHORTS
# =============================================================
print("=" * 60)
...
cohorts = {}
cohort_files = [
    ("GSE26193", "GSE26193_series_matrix.txt"),
    ("GSE9891",  "GSE9891_series_matrix.txt"),
]

for name, filename in cohort_files:
    filepath = os.path.join(DATA_RAW, filename)
    if not os.path.exists(filepath):
        print(f"\nWARNING: {filename} not found in {DATA_RAW}")
        print(f"  Download from GEO and place in dataraw/ folder.")
        continue
    ...
    if pyro is None or treg is None:
        print(f"  SKIPPED — insufficient gene coverage")
        continue
    ...
    print(f"\n  *** KEY RESULT — {name} ***")
    print(f"  Pyroptosis vs Treg: r = {r:.3f}  p = {p:.4f}  n = {len(df)}")
...
# =============================================================
# SCATTER PLOT — GSE9891 (largest validation cohort)
# =============================================================
if "GSE9891" in cohorts:
    ...
    color_map = {
        "High_Pyro_High_Treg":"#1D9E75","High_Pyro_Low_Treg":"#D85A30",
        "Low_Pyro_High_Treg":"#534AB7","Low_Pyro_Low_Treg":"#888780"
    }
    for group, color in color_map.items():
        sub = df[df["Group"]==group]
        ax.scatter(sub["Pyroptosis"], sub["Treg"], c=color, alpha=0.6, s=25,
    ...
    ax.set_xlabel("Pyroptosis score", fontsize=11)
    ax.set_ylabel("Treg score", fontsize=11)
...
# =============================================================
# LOAD TCGA FOR COMPARISON
# =============================================================
print("\nLoading TCGA for cross-cohort comparison...")
tcga_expr   = pd.read_csv(os.path.join(DATA_CLEAN, "TCGA_expr_clean.csv"), index_col=0)
...
pm = tcga_scores["Pyroptosis_score"].median()
tm = tcga_scores["Treg_score"].median()
...
# =============================================================
# IMMUNE CHECKPOINT VALIDATION ACROSS ALL COHORTS
# =============================================================
print("\nValidating immune checkpoint differences...")

def get_checkpoint_expr(expr_gene, scores_df):
    """Extract checkpoint expression matched to patient groups."""
...
# Collect checkpoint data for all cohorts
cohort_data = {}
ck_tcga, grps_tcga = get_checkpoint_expr(tcga_expr, tcga_scores)
if ck_tcga is not None:
    cohort_data["TCGA-OV\n(n=425)"] = (ck_tcga, grps_tcga)

for name, d in cohorts.items():
    ck, grps = get_checkpoint_expr(d["expr"], d["df"].rename(columns={"Group":"Group"}))
    if ck is not None:
        cohort_data[f"{name}\n(n={d['n']})"] = (ck, grps)

all_ck_labels = sorted(set(col for ck_expr, _ in cohort_data.values() for col in ck_expr.columns))
n_ck      = len(all_ck_labels)
n_cohorts = len(cohort_data)

# Plot checkpoint validation
...
print(f"  {'Gene':<10}", end="")
for cl in cohort_data.keys():
    print(f" {cl.split(chr(10))[0]:>12}", end="")
print()
...
for ck_idx, ck_label in enumerate(all_ck_labels):
    ...
    for c_idx, (cohort_label, (ck_expr, grps)) in enumerate(cohort_data.items()):
        ...
        for g, gl, gc in zip(GROUP_ORDER, GROUP_LABELS, GROUP_COLORS):
            ...
            if len(vals) > 0:
                plot_data.append(vals); plot_labels_ax.append(gl); plot_colors_ax.append(gc)

        if len(plot_data) >= 2:
            bp = ax.boxplot(plot_data, patch_artist=True,
                           medianprops=dict(color="black",linewidth=1.5), widths=0.6, showfliers=False)
            for patch, color in zip(bp["boxes"], plot_colors_ax):
            ...
            row_pvals.append("N/A")

        ax.set_xticks(range(1, len(plot_labels_ax)+1))
        ax.set_xticklabels(plot_labels_ax, fontsize=8)
        if ck_idx == 0:
            ax.set_ylabel(f"{cohort_label.split(chr(10))[0]}\nlog2(expr)", fontsize=8)
        sns.despine(ax=ax)
...
legend_elements = [
    Patch(facecolor=GROUP_COLORS[i], alpha=0.7,
          label=f"{'High' if 'High_Pyro' in g else 'Low'} Pyro + {'High' if 'High_Treg' in g else 'Low'} Treg ({GROUP_LABELS[i]})")
...
print("=" * 60)
print(f"  TCGA-OV:   r = 0.813  p < 0.0001  n = 425  (training)")
for name, d in cohorts.items():
    sig = "✓ CONFIRMED" if d["p"] < 0.05 and d["r"] > 0 else "✗ Not significant"
